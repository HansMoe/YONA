...
def train(hyp, opt, device, tb_writer=None):
    ...
    if opt.aug_method == "HFlip":
        transform = transforms.Compose([
            transforms.RandomHorizontalFlip(),
        ])
    elif opt.aug_method == "VFlip":
        transform = transforms.Compose([
            transforms.RandomVerticalFlip(),
        ])
    elif opt.aug_method == "Jitter":
        transform = transforms.Compose([
            transforms.ColorJitter(),
        ])
    elif opt.aug_method == "Erasing":
        transform = transforms.Compose([
            transforms.RandomErasing(),
        ])
    elif opt.aug_method == "Cutout":
        transform = transforms.Compose([
            Cutout(n_holes=5, length=1),
        ])
    elif opt.aug_method == "Grid":
        transform = transforms.Compose([
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar: # batch -------------------------------------------------------------
            ...
            imgs = Mask_Half(imgs, imgs.size(2), imgs.size(3))
            imgs = transform(imgs)
            # Warmup
            if ni <= nw:
                xi = [0, nw]  # x interp
...
if __name__ == '__main__':
    ...
    parser.add_argument('--aug_method', default='HFlip', type=str, help='use what data augmentation method to combine')
